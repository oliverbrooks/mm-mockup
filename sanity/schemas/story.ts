...
export const storyType = defineType({
  ...
  fields: [
    ...
    defineField({ name: 'objectLabel', title: 'Object label', type: 'string' }),
    defineField({ name: 'objectCaption', title: 'Object caption', type: 'text', rows: 2 }),
    defineField({ name: 'objectImage', title: 'Object image', type: 'image', options: { hotspot: true } }),
    defineField({ name: 'featured', title: 'Featured story', type: 'boolean', initialValue: false }),
    defineField({ name: 'publishedAt', title: 'Published at', type: 'datetime' }),
  ],
  preview: {
    select: { title: 'title', subtitle: 'contributor.name', media: 'contributor.portrait' },
  },
})
