...
type Tab = 'individual' | 'institutional' | 'corporate'
...
function IndividualTab() {
  ...
  const [freq, setFreq] = useState<'once' | 'monthly' | 'annual'>('once')
        ...
        <div>
          ...
          {step === 1 && (
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
                {(['once', 'monthly', 'annual'] as const).map((f) => (
                  <button key={f} onClick={() => setFreq(f)} style={{
                    padding: '10px 18px', borderRadius: 999, border: '1.5px solid #000',
                    background: freq === f ? '#000' : '#fff', color: freq === f ? '#fff' : '#000',
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
                ...
                <button onClick={() => setCustom(true)} style={{
                  padding: '22px 0', border: '1.5px solid #000',
                  background: custom ? 'var(--mm-yellow)' : '#fff', fontSize: 22, fontWeight: 700,
                }}>Other</button>
              </div>
              {custom && (
              ...
              <button className="btn btn--big" style={{ marginTop: 32 }} onClick={() => setStep(2)} disabled={!amount}>Continue →</button>
            </div>
          )}
          ...
          {step === 3 && (
            <div style={{ textAlign: 'center', padding: '32px 0' }}>
              <div style={{ width: 80, height: 80, borderRadius: '50%', background: 'var(--mm-green)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 40, fontWeight: 800, marginBottom: 24 }}>✓</div>
          ...
          <div style={{ position: 'sticky', top: 160, background: 'var(--mm-paper)', padding: 32, border: '1.5px solid #000' }}>
            ...
            {[['Amount', `£${amount}`], ['Frequency', freq]].map(([k, v]) => (
              <div key={k} style={{ display: 'flex', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--mm-mid)' }}>
                <span>{k}</span><strong style={{ textTransform: 'capitalize' }}>{v}</strong>
              </div>
            ))}
            {giftAid && (
...
function Field({ label, type = 'text', required, value, onChange }: { label: string; type?: string; required?: boolean; value: string; onChange: (v: string) => void }) {
  return (
    <label style={{ display: 'block' }}>
      ...
      <input type={type} value={value} onChange={(e) => onChange(e.target.value)} required={required}
        style={{ width: '100%', padding: '14px 18px', fontSize: 16, border: '1.5px solid #000', borderRadius: 4, background: '#fff', fontFamily: 'inherit', outline: 'none' }}/>
    </label>
  )
}
      ...
      <div className="layout-donation-grid">
        ...
        <aside>
          <div style={{ background: 'var(--mm-black)', color: '#fff', padding: 32 }}>
            ...
            <p style={{ margin: '0 0 24px', opacity: 0.8 }}>We&apos;d love to discuss how your foundation&apos;s priorities align with our 2028 campaign.</p>
            ...
            <div style={{ marginTop: 24, paddingTop: 24, borderTop: '1px solid rgba(255,255,255,0.2)', fontSize: 14 }}>
              ...
              <strong>Rob Kelly</strong>, Director of Development
            </div>
          </div>
          <div style={{ marginTop: 20 }}>
            <a href="#" className="btn btn--ghost" style={{ width: '100%', justifyContent: 'center' }}>Download prospectus (PDF, 4.2mb)</a>
          </div>
